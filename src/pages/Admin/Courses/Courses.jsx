...
const Courses = () => {
  ...
  useEffect(() => {
    dispatch(
      fetchAllCourses({ page: currentPage, search: debouncedSearchTerm }),
    );
    ...
    try {
      ...
      const courseData = {
        ...
        oldPrice: newCourse.oldPrice
          ? parseFloat(newCourse.oldPrice)
          : undefined,
        image: newCourse.image.file
          ? { file: newCourse.image.file }
          : undefined,
      };
    ...
    try {
      ...
      const courseData = {
        ...
        oldPrice: selectedCourse.oldPrice
          ? parseFloat(selectedCourse.oldPrice)
          : undefined,
        image: selectedCourse.image?.file
          ? { file: selectedCourse.image.file }
          : undefined,
      };
      await dispatch(
        updateCourse({
  ...
  const handleDeleteLesson = async (lessonId) => {
    try {
      ...
      if (result.isConfirmed) {
        await dispatch(
          deleteLesson({ lessonId, courseId: selectedCourse._id }),
        ).unwrap();

        // Refresh lessons list
        const response = await fetch(
          `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/courses/${selectedCourse._id}/lessons`,
          {
            headers: {
  ...
  const handleStatusChange = async (courseId, newStatus) => {
    try {
      await dispatch(
        updateCourseStatus({ courseId, status: newStatus }),
      ).unwrap();
    } catch (error) {
  ...
  const handleGrantAccess = async (course) => {
    ...
    try {
      ...
      await dispatch(
        enrollUserToCourse({ courseId: course._id, email }),
      ).unwrap();
      Swal.fire({
        icon: "success",
      ...
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: err || "Could not grant access",
      });
    } finally {
  ...
  const handleRevokeAccess = async (course) => {
    ...
    try {
      ...
      await dispatch(
        revokeUserFromCourse({ courseId: course._id, email }),
      ).unwrap();
      Swal.fire({
        icon: "success",
      ...
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: err || "Could not revoke access",
      });
    } finally {
  ...
  return (
    <div className="admin-courses">
                    ...
                    <div className="actions-menu">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setOpenActionMenuId(
                            openActionMenuId === course._id ? null : course._id,
                          );
                        }}
                        aria-haspopup="menu"
      ...
      <div className="pagination">
        ...
        <span>
          Page {currentPage} of {courses?.pagination?.totalPages || 1}
        </span>
        <button
            ...
            <form onSubmit={handleCreateCourse}>
              ...
              <div className="form-group checkbox-group">
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    checked={newCourse.requiresForm || false}
                    onChange={(e) =>
                      setNewCourse({
                        ...newCourse,
                      ...
                      <tbody>
                        {section.lessons.map((lesson) => (
                          <tr key={lesson._id}>
                            ...
                            <td>
                              <span
                                className={`status-badge ${lesson.isPrivate ? "private" : "public"}`}
                              >
